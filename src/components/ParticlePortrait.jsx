...
const AsciiPortrait = () => {
  ...
  const processImage = (img, targetSize) => {
    ...
    // --- CROP: head → waist, tuned to a full-body cutout photo ---
    // Nudge these 4 numbers if the framing looks off for your image.
    const cropLeft = 0.28,
      cropRight = 0.77,
      cropTop = 0.35,
      cropBottom = 0.75;
    ...
    const sw = img.width * (cropRight - cropLeft);
    const sh = img.height * (cropBottom - cropTop);
    // ---------------------------------------------------------------
    ...
    offCtx.drawImage(img, sx, sy, sw, sh, offsetX, offsetY, drawWidth, drawHeight);
    const imageData = offCtx.getImageData(0, 0, canvasWidth, canvasHeight);
    const pixels = imageData.data;
  ...
  useEffect(() => {
    ...
    // 1. Check pre-calculated static data
    // --- TEMPORARILY DISABLED while regenerating asciiData from your own photo ---
    // Uncomment this block once you've captured all 3 sizes and pasted them
    // into src/assets/asciiData.js.
    /*
    if (asciiData[size]) {
      particlesRef.current = createParticlesFromRaw(asciiData[size], isMobileSize);
      setDataReady(true);
    ...
    */
    ...
    img.onload = () => {
      ...
      // --- CAPTURE STEP: copy this from the console into asciiData.js ---
      console.log(`SIZE ${size}:`, JSON.stringify(raw));
      // --------------------------------------------------------------------
